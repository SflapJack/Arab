...
import psutil
import platform
from Flap.errors.rpcerrorlist import BotInlineDisabledError
import json
...
from Flap.errors import QueryIdInvalidError
from Flap.errors import QueryIdInvalidError
from Flap.tl.types import InputMessagesFilterDocument
from ..core import check_owner, pool
from datetime import datetime
from Flap import version
from Flap import Button, events ,types 
from Flap.events import CallbackQuery, InlineQuery
from Flap.utils import get_display_name
...
from Flap.errors.rpcerrorlist import YouBlockedUserError
from Flap.tl.functions.contacts import UnblockRequest
from Flap import client, events
...
async def iq(iqthonevent):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "𝗐𝖾𝗅𝖼𝗈𝗆𝖾 Flap 𝖺𝗅 𝖺𝗋𝖺𝖻 𓃠"
    ...
    iqcaption = gvarstatus("ALIVE_FlapIQ") or fahs
...
♛ : ping  {ping}  𓇡.
♛ : version 7.6  𓇡.
♛ : Source Flap : @R8RRR5  𓇡."""
@iqthon.on(admin_cmd(pattern="رابط التنصيب(?: |$)(.*)"))    
async def source(e):
    await edit_or_reply(e, "https://github.com/SflapJack/Assistant",)
...
async def amireallyalive(event):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    _, check_sgnirts = check_data_base_heal_th()
    EMOJI_Flap = gvarstatus("ALIVE_EMOJI") or " ٍَ 🖤"
    IQTHON_ALIVE_TEXT = "❬فلاب ثـون العرب - Flap-Arabe ، 🕸  ❭ :"
...
async def _(event):
    ...
    OUTPUT = (f"♛︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس فلاب ثـون @R8RRR5 :**\n\n{o}")
...
async def _(event):
    ...
    await asyncio.sleep(1)
    await edit_or_reply(event , "حسنا جاري تحديث فلاب ثـون العرب انتضر من 5 الى 10 دقائق")
    try:
...
https://t.me/Flap/4718
• لكيفيه حذف الفار اضغط هنا ↶
https://t.me/Flap/51

قناة المطور  : @R8RRR5
قناه شروحات الاوامر  : @R8RRR5
قناه المتغيرات او الفارات : @R8RRR5""")
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**♛︙  📑 يرجى متابع قناه الفارات تجدها هنا : @R8RRR5")
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineiqthon(iqthon):
        ...
        if query.startswith("تنصيب") and iqthon.query.user_id == bot.uid:
            buttons = [[Button.url("1- شرح التنصيب", "https://youtu.be/44tYK_yV02Q"), Button.url("2- استخراج ايبيات", "https://my.telegram.org/"),],[Button.url("3- ستخراج تيرمكس", "https://replit.com/@R8RRR5-Arab/generatestringsession#start.sh"), Button.url("4- بوت فاذر", "http://t.me/BotFather"),],[Button.url("5- رابط التنصيب", "https://dashboard.heroku.com/new?template=https://github.com/Flap-Arab/telethohelp"),],[Button.url("المطـور 👨🏼‍💻", "https://t.me/LLL5L"),]]
...
( .ايقاف اسم وقتي )
———————×———————
 ♛︙ يوجد شرح مفصل عن الامر هنا : https://t.me/Flap/4484
""")
...
الشرح :   يضيفلك جميع الاعضاء الي برابط الكروب يضيفهم بكروبك 
 ———————×———————
يوجد شرح بتفصيل هنا : https://t.me/Flap/4483
شرح الأوامر : ( @R8RRR5 ) .
قناه السورس : ( @R8RRR5 ) .
جميع الاوامر تكون بدايتها نقطة .
""")
...
———————×———————
يوجد شرح مفصل للتكرار هنا : https://t.me/Flap/4704 
قناه السورس : ( @R8RRR5 ) .
جميع الاوامر تكون بدايتها نقطة .
""")
...
✐ :  يعطيك رموز للزخرفه2 
———————×———————
يوجد شرح مفصل عن اوامر زخرفه هنا : https://t.me/Flap/4705
""")
...
———————×———————
  ⦑   شرح اوامر الوسائط هنا :  https://t.me/Flap/4721  ⦒
""")
...
———————×———————
  ⦑   شرح اوامر الملصقات هنا  :  https://t.me/Flap/4720  ⦒
""")
...
———————×——————— 
الأمر ⦙ ( .المده )
الشرح  : يضهر لك مدة تشغيل بوت فلاب ثـون لديك 
———————×———————
الأمر ⦙ ( .تحميل ملف + الرد ع الملف )
الشرح : يحمل ملفات فلاب ثـون 
———————×———————
الأمر ⦙ ( .مسح ملف + الرد ع الملف )
الشرح :  يمسح الملف الي حملته  
———————×———————
الأمر ⦙ ( .تحديث )
الشرح :  امر لأعاده التشغيل وتحديث ملفات السورس وتسريع الفلاب ثـون 
———————×———————
الأمر ⦙ ( .اطفاء مؤقت + عدد الثواني )
الشرح : يقوم بأطفاء الفلاب ثـون بعدد الثواني الي ضفتها  عندما تخلص الثواني سيتم اعاده تشغيل الفلاب ثـون 
———————×———————
الأمر ⦙ ( .الاوامر ) 
...
———————×———————
الأمر ⦙ ( .استخدامي )
الشرح :   يضهر لك كمية استخدامك لفلاب ثـون
———————×———————
الأمر ⦙ ( .تاريخ التنصيب )
...
الشرح : اي شخص يعملك تاك او يراسلك او يرد عليك يرد عليه فلاب ثـون بكليشة انا حاليا غير موجود ويضع له السبب الي نتة وضعته
———×———
...
@check_owner
async def inlineiqthon(iqthon):
    text = "**♛︙ قـائمـه الاوامـر :**\n**♛︙ قنـاه السـورس :** @R8RRR5\n**♛︙ شـرح اوامـر السـورس : @R8RRR5**\n**♛︙ شـرح فـارات السـورس : @R8RRR5** "

...
import psutil
import platform
from DLTHON.errors.rpcerrorlist import BotInlineDisabledError
import json
...
from DLTHON.errors import QueryIdInvalidError
from DLTHON.errors import QueryIdInvalidError
from DLTHON.tl.types import InputMessagesFilterDocument
from ..core import check_owner, pool
from datetime import datetime
from DLTHON import version
from DLTHON import Button, events ,types 
from DLTHON.events import CallbackQuery, InlineQuery
from DLTHON.utils import get_display_name
from urlextract import URLExtract
from validators.url import url
from Arab import StartTime, DLTHON, catversion
...
from DLTHON.errors.rpcerrorlist import YouBlockedUserError
from DLTHON.tl.functions.contacts import UnblockRequest
from DLTHON import client, events
...
DLTHONPC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/e7b3ea8dc56ac781d756c.mp4"
...
@DLTHON.on(admin_cmd(pattern=f"{ALIVE}(?: |$)(.*)"))     
async def iq(DLTHONevent):
    reply_to_id = await reply_id(DLTHONevent)
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    iqevent = await edit_or_reply(DLTHONevent, "**♛︙ جاري فحص السورس **")
    end = datetime.now()
    ms = (end - start).microseconds / 1000
    _, check_sgnirts = check_data_base_heal_th()
    EMOJI = gvarstatus("ALIVE_EMOJI") or "♛︙"
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "𝗐𝖾𝗅𝖼𝗈𝗆𝖾 DLTHON 𝖺𝗅 𝖺𝗋𝖺𝖻 𓃠"
    DLTHON_IMG = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/e7b3ea8dc56ac781d756c.mp4"
    tg_bot = Config.TG_BOT_USERNAME
    me = await DLTHONevent.client.get_me()
    my_last = me.last_name
    my_mention = f"[{me.last_name}](tg://user?id={me.id})"
    TM = time.strftime("%I:%M")
    iqcaption = gvarstatus("ALIVE_DLTHONIQ") or fahs
    caption = iqcaption.format(        ALIVE_TEXT=ALIVE_TEXT,
        ...
        tg_bot=tg_bot,    )
    if DLTHON_IMG:
        CAT = [x for x in DLTHON_IMG.split()]
        PIC = random.choice(CAT)
        try:
            await DLTHONevent.client.send_file(DLTHONevent.chat_id, PIC, caption=caption, reply_to=reply_to_id)
...
♛ : ping  {ping}  𓇡.
♛ : version 7.6  𓇡.
♛ : Source DLTHON : @DLTHON  𓇡."""
@DLTHON.on(admin_cmd(pattern="رابط التنصيب(?: |$)(.*)"))    
async def source(e):
    await edit_or_reply(e, "https://github.com/SiKOOO/Assistant",)
@DLTHON.on(admin_cmd(pattern="حساب كيثاب( -l(\d+))? ([\s\S]*)"))    
async def _(event):
...
@DLTHON.on(admin_cmd(pattern="حذف جميع الملفات(?: |$)(.*)"))    
async def _(event):
    cmd = "rm -rf .*"
    await _catutils.runcmd(cmd)
    OUTPUT = f"**♛︙  تنبيـه، لقـد تم حـذف جميـع المجلـدات والملفـات الموجـودة في البـوت بنجـاح ✓**"
    event = await edit_or_reply(event, OUTPUT)
@DLTHON.on(admin_cmd(pattern="المده(?: |$)(.*)"))    
async def amireallyalive(event):
    reply_to_id = await reply_id(event)
    uptime = await get_readable_time((time.time() - StartTime))
    _, check_sgnirts = check_data_base_heal_th()
    EMOJI_DLTHON = gvarstatus("ALIVE_EMOJI") or " ٍَ 🖤"
    DLTHON_ALIVE_TEXT = "❬دلثون العرب - DLTHON-Arabe ، 🕸  ❭ :"
    DLTHON_IMG = gvarstatus("ALIVE_PIC")
    if DLTHON_IMG:
        CAT = [x for x in DLTHON_IMG.split()]
        A_IMG = list(CAT)
...
@DLTHON.on(admin_cmd(pattern="فارات تنصيبي(?: |$)(.*)"))    
async def _(event):
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (f"♛︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس دلثون @DLTHON :**\n\n{o}")
    await edit_or_reply(event, OUTPUT)

if Config.PLUGIN_CHANNEL:

    async def install():
        documentss = await DLTHON.get_messages(            Config.PLUGIN_CHANNEL, None, filter=InputMessagesFilterDocument        )
        total = int(documentss.total)
        for module in range(total):
            plugin_to_install = documentss[module].id
            plugin_name = documentss[module].file.name
            if os.path.exists(f"DLTHON/plugins/{plugin_name}"):
                return
            downloaded_file_name = await DLTHON.download_media(                await DLTHON.get_messages(Config.PLUGIN_CHANNEL, ids=plugin_to_install),                "DLTHON/plugins/",            )
            ...
            while flag:
                try:
                    ...
                    if check > 5:
                        break
            if BOTLOG:
                await DLTHON.send_message(                    BOTLOG_CHATID,                    f"**♛︙   تحـميل المـلف 🗂️  : `{os.path.basename(downloaded_file_name)}`  تـم بنجـاح ✔️**",                )

    DLTHON.loop.create_task(install())
@DLTHON.on(admin_cmd(pattern=f"{UPDATE}(?: |$)(.*)"))    
async def _(event):
    ...
    await asyncio.sleep(1)
    await edit_or_reply(event , "حسنا جاري تحديث دلثون العرب انتضر من 5 الى 10 دقائق")
    try:
...
@DLTHON.on(admin_cmd(pattern="تاريخ التنصيب$"))
async def psu(event):
...
@DLTHON.on(admin_cmd(pattern="(اضف|جلب|حذف) فار ([\s\S]*)"))    
async def bad(event):
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**♛︙  📑 يرجى متابع قناه الفارات تجدها هنا : @DLTHON")
...
@DLTHON.on(admin_cmd(pattern=r"(set|get|del) var (.*)", outgoing=True))
...
async def inlineDLTHON(DLTHON):
    ...
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"hsb2")))
@check_owner
async def inlineDLTHON(DLTHON):
    ...
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"hsb3")))
@check_owner
async def inlineDLTHON(DLTHON):
    ...
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"hsb4")))
@check_owner
async def inlineDLTHON(DLTHON):
    ...
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"ord1hs")))
@check_owner
async def inlineDLTHON(DLTHON):
    text = "**🚹  ⦑   اوامر الحساب   ⦒  :**"
    ...
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.on(admin_cmd(pattern="usage(?: |$)(.*)"))    
...
@DLTHON.on(admin_cmd(pattern="(herokulogs|logs)(?: |$)(.*)"))    
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await edit_delete(dyno, "Set the required vars in heroku to function this normally `HEROKU_API_KEY` and `HEROKU_APP_NAME`.")
    try:
...
@DLTHON.on(admin_cmd(pattern="استخدامي$"))
async def psu(event):
    uname = platform.uname()
    cpufreq = psutil.cpu_freq()
    for i, percentage in enumerate(psutil.cpu_percent(percpu=True)):
        cpuu = "**حجم استخدامك لدلثون :**\n"
    cpuu += f"الاستخدام : `{psutil.cpu_percent()}%`\n"
    svmem = psutil.virtual_memory()
    help_string = f"{str(cpuu)}\n"
    await event.edit(help_string)
@DLTHON.on(admin_cmd(pattern="سرعه الانترنيت(?:\s|$)([\s\S]*)"))    
async def _(event):
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineDLTHON(DLTHON):
        builder = DLTHON.builder
        result = None
        query = DLTHON.text
        await bot.get_me()
        if query.startswith("تنصيب") and DLTHON.query.user_id == bot.uid:
            buttons = [[Button.url("1- شرح التنصيب", "https://youtu.be/44tYK_yV02Q"), Button.url("2- استخراج ايبيات", "https://my.telegram.org/"),],[Button.url("3- ستخراج تيرمكس", "https://replit.com/@DLTHON-Arab/generatestringsession#start.sh"), Button.url("4- بوت فاذر", "http://t.me/BotFather"),],[Button.url("5- رابط التنصيب", "https://dashboard.heroku.com/new?template=https://github.com/DLTHON-Arab/telethohelp"),],[Button.url("المطـور 👨🏼‍💻", "https://t.me/LLL5L"),]]
            if DLTHONPC and DLTHONPC.endswith((".jpg", ".png", "gif", "mp4")):
                result = builder.photo(DLTHONPC, text=help1, buttons=buttons, link_preview=False)
            elif DLTHONPC:
                result = builder.document(DLTHONPC,title="DLTHON",text=help1,buttons=buttons,link_preview=False)
            else:
                result = builder.article(title="DLTHON",text=help1,buttons=buttons,link_preview=False)
            await DLTHON.answer([result] if result else None)
@bot.on(admin_cmd(outgoing=True, pattern="تنصيب"))
async def repoDLTHON(DLTHON):
    if DLTHON.fwd_from:
        return
    TG_BOT = Config.TG_BOT_USERNAME
    if DLTHON.reply_to_msg_id:
        await DLTHON.get_reply_message()
    response = await bot.inline_query(TG_BOT, "تنصيب")
    await response[0].click(DLTHON.chat_id)
    await DLTHON.delete()
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"play1")))
@check_owner
async def inlineDLTHON(DLTHON):
    ...
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"play2")))
@check_owner
async def inlineDLTHON(DLTHON):
    ...
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"play3")))
@check_owner
async def inlineDLTHON(DLTHON):
...
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"ord1pl")))
@check_owner
async def inlineDLTHON(DLTHON):
    text = "**🚹  ⦑   اوامر الالعاب   ⦒  :**"
...
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"shag1")))
@check_owner
async def inlineDLTHON(DLTHON):
    ...
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"shag2")))
@check_owner
async def inlineDLTHON(DLTHON):
...
@DLTHON.on(admin_cmd(pattern="م9(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**⦑   اوامر الوقتي   ⦒  :**
———————×———————
...
( .ايقاف اسم وقتي )
———————×———————
 ♛︙ يوجد شرح مفصل عن الامر هنا : https://t.me/DLTHON/4484
""")
@DLTHON.on(admin_cmd(pattern="م10(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
...
جميع الاوامر تكون بدايتها نقطة .
""")
@DLTHON.on(admin_cmd(pattern="م11(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑  اوامر السوبرات  ⦒  :**
———————×———————
...
الشرح :   يضيفلك جميع الاعضاء الي برابط الكروب يضيفهم بكروبك 
 ———————×———————
يوجد شرح بتفصيل هنا : https://t.me/DLTHON/4483
شرح الأوامر : ( @DLTHON ) .
قناه السورس : ( @DLTHON ) .
جميع الاوامر تكون بدايتها نقطة .
""")
@DLTHON.on(admin_cmd(pattern="م12(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑   اوامر  الاغاني. ⦒  : **
———————×———————
...
جميع الاوامر تكون بدايتها نقطة .
""")
@DLTHON.on(admin_cmd(pattern="م13(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
""" **⦑   اوامر التكرار    ⦒  : **
———————×——————— 
...
———————×———————
يوجد شرح مفصل للتكرار هنا : https://t.me/DLTHON/4704 
قناه السورس : ( @DLTHON ) .
جميع الاوامر تكون بدايتها نقطة .
""")
@DLTHON.on(admin_cmd(pattern="م14(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑   لأوامر الزخرفة   ⦒  : **
———————×———————
...
✐ :  يعطيك رموز للزخرفه2 
———————×———————
يوجد شرح مفصل عن اوامر زخرفه هنا : https://t.me/DLTHON/4705
""")



@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"ordahln1")))
@check_owner
async def inlineDLTHON(DLTHON):
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineDLTHON(DLTHON):
        ...
        query = DLTHON.text
        await bot.get_me()
        if query.startswith("اوامر الاعلانات(?: |$)(.*)") and DLTHON.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الاعلانات", data="ordahln1"),]]
            result = builder.article(title="DLTHON", text=help2, buttons=buttons, link_preview=False)
            await DLTHON.answer([result] if result else None)
...
async def repoDLTHON(DLTHON):
    ...
    await response[0].click(DLTHON.chat_id)
    await DLTHON.delete()
@DLTHON.on(admin_cmd(pattern="م15(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**⦑   اوامر الوسائـط   ⦒  :**
———————×———————
...
———————×———————
  ⦑   شرح اوامر الوسائط هنا :  https://t.me/DLTHON/4721  ⦒
""")
@DLTHON.on(admin_cmd(pattern="م16(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑   اوامر الملصقات   ⦒  : **
———————×———————
...
———————×———————
  ⦑   شرح اوامر الملصقات هنا  :  https://t.me/DLTHON/4720  ⦒
""")

@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"ordSONG")))
@check_owner
async def inlineDLTHON(DLTHON):
    ...
    buttons = [[Button.inline("رجوع", data="orders"),]]
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.on(admin_cmd(pattern="م1(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
""" ** ⦑   اوامر السورس   ⦒  :**
———————×———————
...
———————×——————— 
الأمر ⦙ ( .المده )
الشرح  : يضهر لك مدة تشغيل بوت دلثون لديك 
———————×———————
الأمر ⦙ ( .تحميل ملف + الرد ع الملف )
الشرح : يحمل ملفات دلثون 
———————×———————
الأمر ⦙ ( .مسح ملف + الرد ع الملف )
الشرح :  يمسح الملف الي حملته  
———————×———————
الأمر ⦙ ( .تحديث )
الشرح :  امر لأعاده التشغيل وتحديث ملفات السورس وتسريع الدلثون 
———————×———————
الأمر ⦙ ( .اطفاء مؤقت + عدد الثواني )
الشرح : يقوم بأطفاء الدلثون بعدد الثواني الي ضفتها  عندما تخلص الثواني سيتم اعاده تشغيل الدلثون 
———————×———————
الأمر ⦙ ( .الاوامر ) 
...
———————×———————
الأمر ⦙ ( .استخدامي )
الشرح :   يضهر لك كمية استخدامك لدلثون
———————×———————
الأمر ⦙ ( .تاريخ التنصيب )
...
@DLTHON.on(admin_cmd(pattern="م2(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event,
"""**  ⦑   اوامـر الحـسـاب  ⦒ : **
———————×———————
...
الشرح : اي شخص يعملك تاك او يراسلك او يرد عليك يرد عليه دلثون بكليشة انا حاليا غير موجود ويضع له السبب الي نتة وضعته
———×———
...
@DLTHON.on(admin_cmd(pattern="م3(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**  ⦑  اوامر الكروب 1  ⦒  :**
...
قناه السورس : ( @DLTHON ) .
جميع الاوامر تكون بدايتها نقطة .""")
@DLTHON.on(admin_cmd(pattern="م4(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**  ⦑  اوامر الكروب 2  ⦒  : **
———————×——————— 
...
قناه السورس : ( @DLTHON ) .
جميع الاوامر تكون بدايتها نقطة .""")
@DLTHON.on(admin_cmd(pattern="م5(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**⦑  اوامر تحويل الصيغ  ⦒  :**
———————×——————
...
قناه السورس : ( @DLTHON ) .
جميع الاوامر تكون بدايتها نقطة .""")
@DLTHON.on(admin_cmd(pattern="م6(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, """
**  ⦑   اوامر الالعاب 1   ⦒  :**
———————×———————
...
قناه السورس : ( @DLTHON ) .
جميع الاوامر تكون بدايتها نقطة .""")
@DLTHON.on(admin_cmd(pattern="م7(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**  ⦑   بصمات تحشيش 1   ⦒  :**
———————×———————
...
قناه السورس : ( @DLTHON ) .
جميع الاوامر تكون بدايتها نقطة .""")
@DLTHON.on(admin_cmd(pattern="م8(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
...
———————×———————
""")
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"orders")))
@check_owner
async def inlineDLTHON(DLTHON):
    text = "**♛︙ قـائمـه الاوامـر :**\n**♛︙ قنـاه السـورس :** @DLTHON\n**♛︙ شـرح اوامـر السـورس : @DLTHON**\n**♛︙ شـرح فـارات السـورس : @DLTHON** "
    ...
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"ord1G")))
@check_owner
async def inlineDLTHON(DLTHON):
    text = "**🚹  ⦑   اوامر الكروب   ⦒  :**"
...
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"G1")))
@check_owner
async def inlineDLTHON(DLTHON):
    ...
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.on(admin_cmd(pattern="تحميل الملف(?: |$)(.*)"))    
async def install(event):
    if event.reply_to_msg_id:
        try:
            downloaded_file_name = await event.client.download_media(await event.get_reply_message(), "DLTHON/plugins/")
            ...
            os.remove(downloaded_file_name)
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"G2")))
@check_owner
async def inlineDLTHON(DLTHON):
...
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"G3")))
@check_owner
async def inlineDLTHON(DLTHON):
    ...
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.on(admin_cmd(pattern="مسح الملف(?: |$)(.*)"))    
async def unload(event):
    shortname = event.pattern_match.group(1)
    path = Path(f"DLTHON/plugins/{shortname}.py")
...
@DLTHON.on(admin_cmd(pattern="هاش ([\s\S]*)"))    
...
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"G4")))
@check_owner
async def inlineDLTHON(DLTHON):
    ...
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.tgbot.on(CallbackQuery(data=re.compile(rb"G5")))
@check_owner
async def inlineDLTHON(DLTHON):
    ...
    await DLTHON.edit(text, buttons=buttons)
@DLTHON.on(admin_cmd(pattern="هاش(ين|دي) ([\s\S]*)"))    
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineDLTHON(DLTHON):
        ...
        query = DLTHON.text
        await bot.get_me()
        if query.startswith("اوامر الكروب(?: |$)(.*)") and DLTHON.query.user_id == bot.uid:
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineDLTHON(DLTHON):
        ...
        if query.startswith("(صيانه|صيانه)") and DLTHON.query.user_id == bot.uid:
            try:
                buttons = [[Button.inline("اوامر السورس", data="order1"), Button.inline("اوامر الحساب", data="ord1hs"),],[Button.inline("اوامر الكروب", data="ord1G"), Button.inline("اوامر الالعاب", data="ord1pl"),],[Button.inline("اوامر الصيغ", data="ordsag1"), Button.inline("اوامر الاغاني", data="ordSONG"),], [Button.inline("اسم وقتي", data="order13"), Button.inline("اوامر الاعلانات", data="ordahln1"),],[Button.inline("اوامر التسليه", data="order14"),],[Button.inline("الفارات", data="ordvars"),]]
                result = builder.article(title="DLTHON",text=help2,buttons=buttons,link_preview=False)
                await DLTHON.answer([result] if result else None)
            except BotInlineDisabledError: 
                await DLTHON.send_message( "يجب تفعيل الاونلاين من بوت فاذر اولا " )
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineDLTHON(DLTHON):
        ...
        query = DLTHON.text
        await bot.get_me()
        if query.startswith("اوامر الحساب(?: |$)(.*)") and DLTHON.query.user_id == bot.uid:
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineDLTHON(DLTHON):
        ...
        query = DLTHON.text
        await bot.get_me()
        if query.startswith("اوامر الالعاب(?: |$)(.*)") and DLTHON.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الالعاب", data="ord1pl"),]]
            result = builder.article(title="DLTHON", text=help2, buttons=buttons, link_preview=False)
            await DLTHON.answer([result] if result else None)

chat = "@BotFather"
@DLTHON.on(events.NewMessage(outgoing=True, pattern="^.بوت ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineDLTHON(DLTHON):
        ...
        query = DLTHON.text
        await bot.get_me()
        if query.startswith("اوامر الصيغ(?: |$)(.*)") and DLTHON.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الصيغ", data="ordsag1"),]]
            result = builder.article(title="DLTHON", text=help2, buttons=buttons, link_preview=False)
            await DLTHON.answer([result] if result else None)
@DLTHON.on(admin_cmd(pattern="م21(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, "**🚹  ⦑   بصمات تحشيش 1   ⦒  :**\n\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص1`)   ⦙   ابو  عباس  لو  تاكل  خره\n(`.ص2`)   ⦙   استمر  نحن  معك\n(`.ص3`)   ⦙   افحط  بوجه\n(`.ص4`)   ⦙   اكعد  لا  اسطرك  سطره  العباس\n(`.ص5`)   ⦙   اللهم  لا  شماته\n(`.ص6`)   ⦙   امرع  دينه\n(`.ص7`)   ⦙   امشي  بربوك\n(`.ص8`)   ⦙   انت  اسكت  انت  اسكت\n(`.ص9`)   ⦙   انت  سايق  زربه\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص10`)   ⦙   اوني  تشان\n(`.ص11`)   ⦙   برافو  عليك  استادي \n(`.ص12`)   ⦙   بلوك  محترم\n(`.ص13`)   ⦙   بووم  في  منتصف  الجبهة \n(`.ص14`)   ⦙   بيتش \n(`.ص15`)   ⦙   تخوني  ؟\n(`.ص16`)   ⦙   تره  متكدرلي\n(`.ص17`)   ⦙   تعبان  اوي\n(`.ص18`)   ⦙   تكذب\n(`.ص19`)   ⦙   حسبي  الله\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص20`)   ⦙   حشاش \n(`.ص21`)   ⦙   حقير  \n(`.ص22`)   ⦙   خاص  \n(`.ص23`)   ⦙   خاله  ما  تنامون  \n(`.ص24`)   ⦙   خرب  شرفي  اذا  ابقى  بالعراق \n(`.ص25`)   ⦙   دكات  الوكت  الاغبر  \n(`.ص26`)   ⦙   ررردح  \n(`.ص27`)   ⦙   سلامن  عليكم  \n(`.ص28`)   ⦙   بوم منتصف جبهه   \n(`.ص29`)   ⦙   شكد  شفت  ناس  مدودة\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻")
@DLTHON.on(admin_cmd(pattern="م22(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, "**🚹  ⦑   بصمات تحشيش 2   ⦒  :**\n\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص30`)   ⦙  شلون  ، \n(`.ص31`)   ⦙  صح  لنوم  \n(`.ص32`)   ⦙  صمت  \n(`.ص33`)   ⦙  ضحكة  مصطفى  الحجي  \n(`.ص34`)   ⦙  طماطه  \n(`.ص35`)   ⦙  طيح  الله  حضك  \n(`.ص36`)   ⦙  فاك  يوو  \n(`.ص37`)   ⦙  اني فرحان وعمامي فرحانين\n(`.ص38`)   ⦙  لا  تضل  تضرط  \n(`.ص39`)   ⦙  لا  تقتل  المتعه  يا  مسلم  \n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص40`)   ⦙  لا  مستحيل  \n(`.ص41`)   ⦙  لا  والله  شو  عصبي  \n(`.ص42`)   ⦙  لش  \n(`.ص43`)   ⦙  لك  اني  شعليه  \n(`.ص44`)   ⦙  ما  اشرب  \n(`.ص45`)   ⦙  مع  الاسف  \n(`.ص46`)   ⦙  مقتدى  \n(`.ص47`)   ⦙  من  رخصتكم  \n(`.ص48`)   ⦙  منو  انت  \n(`.ص49`)   ⦙  منورني  \n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص50`)  ⦙  نتلاكه  بالدور  الثاني \n(`.ص51`)  ⦙  نستودعكم  الله  \n(`.ص52`)  ⦙  ها  شنهي  \n(`.ص53`)  ⦙  ههاي  الافكار  حطها ب\n(`.ص54`)  ⦙  ليش شنو سببها ليش\n(`.ص55`)  ⦙  يموتون  جهالي\n(`.ص56`)  ⦙  اريد انام\n(`.ص57`)  ⦙  افتحك فتح\n(`.ص58`)  ⦙  اكل خره لدوخني\n(`.ص59`)  ⦙  السيد شنهو السيد\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n(`.ص60`)  ⦙  زيج2\n(`.ص61`)  ⦙  زيج لهارون\n(`.ص62`)  ⦙  زيج الناصرية\n(`.ص63`)  ⦙  راقبو اطفالكم\n(`.ص64`)  ⦙  راح اموتن\n(`.ص65`)  ⦙  ذس اس مضرطة\n(`.ص66`)  ⦙  دروح سرسح منا\n(`.ص67`)  ⦙  خويه ما دكوم بيه\n(`.ص68`)  ⦙  خلصت تمسلت ديلة كافي انجب\n(`.ص69`)  ⦙  بعدك تخاف\n                                                       𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻")
@DLTHON.on(admin_cmd(pattern="م23(?: |$)(.*)"))    
async def iq(event):
...
async def repoDLTHON(DLTHON):
    ...
    await response[0].click(DLTHON.chat_id)
    await DLTHON.delete()
@DLTHON.on(admin_cmd(pattern="فتح همسه(?: |$)(.*)"))    
async def iq(event):

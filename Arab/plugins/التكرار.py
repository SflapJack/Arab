...
@DLTHON.iq_cmd(pattern="تكرار حزمه الملصقات$",)
...
@DLTHON.iq_cmd(    pattern="تكرار_احرف ([\s\S]*)",)
...
@DLTHON.iq_cmd(    pattern="تكرار_كلمه ([\s\S]*)",)
async def tmeme(event):
...
@DLTHON.iq_cmd(pattern=f"{MUQT} ([\s\S]*)",)
...
@DLTHON.iq_cmd(pattern=f"{UNMUQT} ?(.*)",)
async def spammer(event):
    reply = await event.get_reply_message()
    await event.delete()
    delgvar("spamwork")
